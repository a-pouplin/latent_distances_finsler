...
from finsler.utils.helper import pickle_save, pickle_load
...
if __name__ == '__main__':
    ...
    if opts.train:
        ...
        if opts.n_samples < 16: Xu = stats.resample(X.clone(), opts.n_samples)
        else: Xu = stats.resample(X.clone(), 16)
    ...
    for i in range(opts.num_geod):
        ...
        result_finsler = geodesic_minimizing_energy(spline_finsler, gplvm_finsler, max_iter=20, eval_grid=20) # eval_grid<20 to avoid numerical problems
    ...
    ax.scatter3D(y_random[:,0], y_random[:,1], y_random[:,2], c='green', s=5, label='random latent data') # random points taken from the latent
    ax.legend()
