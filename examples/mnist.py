...
if __name__ == "__main__":
    ...
    if opts.save_model:
        ...
        manifold.fit(model=gplvm, grid=[ran, ran], batch_size=512)
    ...
    # colors_data = sns.color_palette("dark", n_colors=9)
    ...
    # if opts.mode == 'riemannian':
    #     spline_manifold.plot(color='purple', linewidth=1, linestyle='--')
    # elif opts.mode == 'finslerian':
    #     spline_manifold.plot(color='orange', linewidth=1, linestyle='--')
    # spline_euclidean.plot(color='k', linewidth=1, alpha=0.7)
    ...
    plt.ylim(-1, 1)
    plt.show()
    filename = "latent_mnist_images_{}.png".format(opts.mode)
    filepath = os.path.join(opts.model_folder, filename)
