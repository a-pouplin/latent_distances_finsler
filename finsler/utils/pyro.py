...
from finsler.utils.data import make_sphere_points
...
def iteration(model, optimizer, num_iter, n_samples):
    ...
    for iter in range(num_iter):
        ...
        if iter % 100 == 0:
            print("Batch: {}/{} - loss {:.2f} -  time {:.3f}".format(
                  iter, num_iter, loss, _time), end='\r')
    return losses
...
def initialise_kernel(data):
    if data == 'sphere':
        ...
        for i, mu in enumerate(mus):
            Y[num_samples*i:num_samples*(i+1)] = sample_vMF(mu=mu, kappa = 50., num_samples=num_samples)
        Y = torch.tensor(Y, dtype=torch.float32)
        X = get_prior(Y=Y, init_name='iso')
        ...
        file = opts.letter
        mat = spio.loadmat('../data/fonts_matlab/{}.mat'.format(file), squeeze_me=True)
        Y, X, lengthscale, variance, noise = font_prior(mat)
