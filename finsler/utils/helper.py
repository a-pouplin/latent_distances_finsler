...
def grid_latent(X, model, n_grid=25):
    ...
    ux, uy = np.meshgrid(np.linspace(Xmin[0], Xmax[0], n_grid),
                         np.linspace(Xmin[1], Xmax[1], n_grid))
...
def indicatrices(model, points, mode='riemann'):
    ...
    for nn in range(n_of_points):
        ...
        vecy = torch.linspace(-coeff*alpha, coeff*alpha, vec_size)
        ...
        cs = axc.contour(indicatrix.detach().numpy(), (1.,))
        plt.close(figc)
        sgm = cs.collections[0].get_segments()[0]
        eps = 1e-2
        if (abs(sgm[0,0] - sgm[-1,0]) < eps) and (abs(sgm[0,1] - sgm[-1,1]) < eps) and len(cs.collections[0].get_segments())==1:
...
def volume_heatmap(ax, model, X, mode, n_grid=25, log=True):
    ...
    if torch.is_tensor(xx): xx = xx.detach().numpy()
    if log: map = np.log10(map)

    grid = griddata(xx, map, (ux, uy), method='nearest')
    im = ax.imshow(grid, extent=(Xmin[0], Xmax[0], Xmin[1], Xmax[1]),
                   origin='lower', cmap='RdBu', aspect='auto', interpolation='bicubic')
...
def contour_riemann(finsler, riemann, vectors, out_dir):
    ...
    axs.legend(handles=[proxy1, proxy2],
               labels=['finsler', 'riemann'])
    axs.set_xticks([])
    axs.set_yticks([])
...
def ellipse_draw(matrix, center, scale, **kwargs):
    ...
    if torch.is_tensor(matrix): matrix = torch.squeeze(matrix).detach().numpy()
    if torch.is_tensor(center): center = torch.squeeze(center).detach().numpy()
    width, height, angle = ellipse_params(matrix)
    return Ellipse(center, width=scale*width, height=scale*height,
...
def automated_scaling(metric):
    ...
    """
    if torch.is_tensor(metric): metric = torch.squeeze(metric).detach().numpy()
    eigvalues, _ = np.linalg.eig(metric)
    ratio = np.max(eigvalues)/np.min(eigvalues)
    # this is awful and arbitrary
    if ratio<10: vec_size = 8
    elif 10<ratio<50: vec_size = 16
    elif 50<ratio<200: vec_size = 32
    elif 200<ratio: vec_size = 64
    else: vec_size = 16
...
def automated_scaling2(metric):
    ...
    if (theta%180)<45 or (theta%180)>135: return long_size, 2*short_size
    elif 45<(theta%180)<135 or (theta%180)>135: return 2*short_size, long_size
# def indicatrices_on_geodesic_old(ax, model, points, scale=1.0):
    ...
    #     ax.add_patch(ellipse_draw(metrics_riemann[i], points[i], scale=scale/len_vec, linewidth=1, edgecolor='tab:blue'))
